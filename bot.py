...
@client.command(name='add-location',
                help='Given a building code and room number, adds a new study location. e.g. /add_location CSE2 114',
                brief='Adds a new study location')
async def add_location(ctx, building, room_no):
  location = f'{building.lower()}-{room_no}'
  if location in LOCATIONS:
    await ctx.channel.send('Location already exists. Please use the `tag_location` command to tag yourself!')
    return
  else:
  ...
  await channel.send(f'Location {location} added!')
